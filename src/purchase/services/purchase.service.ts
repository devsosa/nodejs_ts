...
export class PurchaseService extends BaseService<PurchaseEntity> {
  ...
  async findPurchaseById(id: number): Promise<PurchaseEntity | null> {
  ...
  async deletePurchase(id: number): Promise<DeleteResult> {
    return (await (this.execRepository)).delete({id});
  }
  async updatePurchase(id: number, infoUpdate: PurchaseDTO): Promise<UpdateResult> {
